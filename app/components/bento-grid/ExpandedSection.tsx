  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-mono text-xs uppercase tracking-widest text-zinc-300">
          {title}
        </h3>
      </div>
      {children}
...
export function DetailItem({ label, text }: { label: string; text: string }) {
  return (
    <div className="mb-3 last:mb-0">
      <dt className="text-xs font-medium text-zinc-300">{label}</dt>
      <dd className="mt-1 text-[13px] leading-relaxed text-zinc-500">
        {text}
      </dd>
    </div>
  );
...
export function PipelineStep({
  ...
  return (
    <div className={`relative border-l-2 ${accentColor} py-2 pl-4`}>
      ...
      <p className="mt-0.5 text-sm font-medium text-zinc-200">{title}</p>
