...
import { fallbackInvoiceMasterProject } from "@/lib/fallback-data";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        {data.badge && (
          <span className="hidden shrink-0 rounded-md border border-amber-500/20 bg-amber-500/10 px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider text-amber-300 sm:inline-flex">
            {data.badge}
          </span>
        )}
      </div>
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between md:mt-5">
        ...
        {data.sourceUrl && (
          <a
            ...
            className="group inline-flex w-fit items-center gap-1.5 rounded-lg border border-white/[0.08] bg-white/[0.04] px-4 py-2 font-mono text-xs text-ink-secondary transition-all active:bg-amber-500/10 md:hover:border-amber-500/40 md:hover:bg-amber-500/10 md:hover:text-amber-300 md:hover:shadow-[0_0_20px_-6px_rgba(245,158,11,0.3)]"
          >
            View Source
      ...
      {/* Decorative accent */}
      <div className="pointer-events-none absolute -bottom-16 -right-16 h-48 w-48 rounded-full bg-amber-500/[0.06] blur-3xl" />
    </BentoCard>
  );
}
