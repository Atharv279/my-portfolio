...
// Aceternity's original PinContainer wraps everything in <a> and also nests an
// <a> for the title pill — invalid HTML. Both are <div> here; the surrounding
// BentoCard already handles activation, and the pill is purely decorative.
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{ transform }}
          className="absolute left-1/2 top-1/2 flex items-start justify-start overflow-hidden rounded-2xl border border-hairline bg-background p-4 shadow-[0_8px_16px_rgb(0_0_0/0.4)] transition duration-700 group-hover/pin:border-hairline-strong"
        >
...
type PinPerspectiveProps = { title?: string };

export const PinPerspective: React.FC<PinPerspectiveProps> = ({ title }) => {
  return (
    <motion.div className="pointer-events-none flex h-80 w-96 items-center justify-center opacity-0 transition duration-500 group-hover/pin:opacity-100 z-[60]">
      <div className="-mt-7 h-full w-full flex-none inset-0">
        <div className="absolute inset-x-0 top-0 flex justify-center">
          <div className="relative z-10 flex items-center space-x-2 rounded-full bg-background px-4 py-0.5 ring-1 ring-hairline">
            <span className="relative z-20 inline-block py-0.5 text-xs font-bold text-ink">
              {title}
            </span>
