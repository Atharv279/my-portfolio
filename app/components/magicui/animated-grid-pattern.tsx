...
export function AnimatedGridPattern({
      ...
      resizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          ...
          setSquares((current) => {
            ...
            return generateSquares(numSquares, nextWidth, nextHeight);
          });
        }
      });
  ...
  return (
    <svg
      ref={containerRef}
      aria-hidden="true"
      className={`pointer-events-none absolute inset-0 h-full w-full stroke-[color:var(--color-hairline-strong)] text-[color:var(--color-hairline-strong)] ${className ?? ""}`}
      {...props}
    >
      <defs>
        <pattern
